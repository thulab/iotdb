...
public class IndexTest {
	...
	public void testSelectIndex() throws QueryProcessorException, ArgsErrorException{
		String sql = "select index subsequence_matching(root.laptop.d1.s1, 'query.csv',123.1)";
		QueryProcessor processor = new QueryProcessor(new MemIntQpExecutor());
		...
		assertEquals("query.csv", indexQueryPlan.getCsvPath());
	}
	@Test
	public void testSelectIndex2() throws QueryProcessorException, ArgsErrorException{
		String sql = "select index subsequence_matching(root.laptop.d1.s1, 'query.csv',123.1,11.1,22.2) where time>100 and time<200 ";
		QueryProcessor processor = new QueryProcessor(new MemIntQpExecutor());
		IndexQueryPlan indexQueryPlan = (IndexQueryPlan) processor.parseSQLToPhysicalPlan(sql);
		assertEquals("root.laptop.d1.s1", indexQueryPlan.getPaths().get(0).getFullPath());
		assertEquals(123.1, indexQueryPlan.getEpsilon(),1E-2);
	...
	public void testSelectIndex3() {
		String sql = "select index subsequence_matching(root.laptop.d1.s1, 'query.csv',123.1,11.1,22.2) where time>200 and time<100 ";
		QueryProcessor processor = new QueryProcessor(new MemIntQpExecutor());
		...
		try {
			...
			assertEquals("for index query command, the time filter must be a interval",e.getMessage());
		}
		sql = "select index subsequence_matching(root.laptop.d1.s1, 'query.csv',123.1,11.1,22.2) where time<-1 and time>-100";
		try {
			indexQueryPlan = (IndexQueryPlan) processor.parseSQLToPhysicalPlan(sql);
			assertEquals("root.laptop.d1.s1", indexQueryPlan.getPaths().get(0).getFullPath());
		} catch (QueryProcessorException | ArgsErrorException e) {
		...
		sql = "select index subsequence_matching(root.laptop.d1.s1, 'query.csv',123.1,11.1,22.2) where time<100";
		try {
			indexQueryPlan = (IndexQueryPlan) processor.parseSQLToPhysicalPlan(sql);
			assertEquals("root.laptop.d1.s1", indexQueryPlan.getPaths().get(0).getFullPath());
