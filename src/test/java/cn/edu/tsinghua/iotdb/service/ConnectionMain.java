...
public class ConnectionMain {
    ...
    static String errorSelect = "SELECT s_71 FROM root.performf.group_6.d_65 where s_71 > 0.0";
    static String errorCount = "SELECT count(s_71) FROM root.performf.group_6.d_65 where s_71 > 0.0";
...
//            statement.close();
            ...
            statement = connection.createStatement();
            hasResultSet = statement.execute(errorCount);
            if (hasResultSet) {
                ResultSet resultSet = statement.getResultSet();
                int cnt = 0;
                while (resultSet.next()) {
