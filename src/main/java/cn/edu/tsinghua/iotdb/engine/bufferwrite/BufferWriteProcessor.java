...
public class BufferWriteProcessor extends Processor {
	...
	public synchronized TombstoneFile getTombstoneFile() throws IOException {
		if(tombstoneFile == null) {
			tombstoneFile = TombstoneFileFactory.getFactory().getTombstoneFile(bufferWriteResource.getInsertFilePath() + TombstoneFile.TOMBSTONE_SUFFIX);
		}
	...
	public void appendTombstone(String deltaObjectId, String measurementId, long timeUpperBound) throws IOException {
		if (bufferWriteResource.hasTimeseries(deltaObjectId, measurementId, timeUpperBound)) {
