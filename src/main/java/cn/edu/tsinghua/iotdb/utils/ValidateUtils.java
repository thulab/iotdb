...
public class ValidateUtils {
    ...
    public static void validatePassword(String password) throws AuthException {
        if(password.length() < MIN_PASSWORD_LENGTH)
            throw new AuthException("Password's length must be greater than or equal to 6");
    }

    public static void validateUsername(String username) throws AuthException {
        if(username.length() < MIN_USERNAME_LENGTH)
            throw new AuthException("Username's length must be greater than or equal to 6");
    }
    ...
    public static void validatePath(String path) throws AuthException {
        if(!path.startsWith(ROOT_PREFIX))
            throw new AuthException(String.format("Illegal path %s, path should start with \"root\"", path));
    }
