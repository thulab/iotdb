...
public class JMXService implements IService {
    ...
    private JMXConnectorServer createJMXServer(int port, boolean local) throws IOException {
        ...
        if (local) {
            ...
            System.setProperty(TsfileConstant.RMI_SERVER_HOST_NAME, serverAddress.getHostAddress());
        }
        int rmiPort = Integer.getInteger(TsfileConstant.JMX_REMOTE_RMI_PORT, 0);
	...
	@Override
	public void start() {
		if (System.getProperty(TsfileConstant.REMOTE_JMX_PORT_NAME) != null) {
			LOGGER.warn("JMX settings in conf/{}.sh(Unix or OS X, if you use Windows, check conf/{}.bat) have been bypassed as the JMX connector server is "
					...
					TsfileConstant.ENV_FILE_NAME, TsfileConstant.ENV_FILE_NAME, TsfileConstant.ENV_FILE_NAME);
			return;
		}
		System.setProperty(TsfileConstant.SERVER_RMI_ID, "true");
		boolean localOnly = false;
		String jmxPort = System.getProperty(TsfileConstant.TSFILEDB_REMOTE_JMX_PORT_NAME);
		...
		try {
			...
			if (jmxService == null)
				return;
			jmxService.start();
	        LOGGER.info("{}: start {} successfully.", TsfileConstant.GLOBAL_DB_NAME,  this.getID().getName());
		} catch (IOException e) {
