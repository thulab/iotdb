...
//        }
    ...
    private DynamicOneColumnData readOneColumnWithoutFilter(Path path, DynamicOneColumnData res, int fetchSize) throws ProcessorException, IOException, PathErrorException {
        ...
        RecordReader recordReader = RecordReaderFactory.getInstance().getRecordReader(0, deltaObjectID, measurementID, null, null, null);

        if (res == null) {
            // get four overflow params
            List<Object> params = getOverflowInfoMergeWithBwMemPage(null, null, null,
...
//        }
    ...
    private DynamicOneColumnData readOneColumnUseFilter(Path path, SingleSeriesFilterExpression timeFilter,
        ...
        RecordReader recordReader = RecordReaderFactory.getInstance().getRecordReader(0, deltaObjectId, measurementId, timeFilter, freqFilter, valueFilter);

        if (res == null) {
            // get four overflow params
            List<Object> params = getOverflowInfoMergeWithBwMemPage(timeFilter, freqFilter, valueFilter,
                    res, recordReader.insertPageInMemory, recordReader.overflowInfo);
