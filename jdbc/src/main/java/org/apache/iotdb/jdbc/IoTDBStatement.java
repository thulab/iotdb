...
public class IoTDBStatement implements Statement {
    ...
    /**
     * Sets the limit for the maximum number of rows that any ResultSet object produced by this Statement can contain to
     * the given number. If the limit is exceeded, the excess rows are silently dropped. The value must be >= 0, and 0
    ...
    private boolean executeSQL(String sql) throws TException, SQLException {
        ...
        if (sqlToLowerCase.startsWith(SHOW_TIMESERIES_COMMAND_LOWERCASE)) {
            ...
            if (execResp.getOperationHandle().hasResultSet) {
                resultSet = new IoTDBQueryResultSet(this, execResp.getColumns(), client, sessionHandle, operationHandle,
