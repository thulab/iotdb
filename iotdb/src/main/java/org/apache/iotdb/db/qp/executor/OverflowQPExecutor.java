...
public class OverflowQPExecutor extends QueryProcessExecutor {
    ...
    @Override
    public QueryDataSet aggregate(List<Pair<Path, String>> aggres, IExpression expression) throws ProcessorException {
        ...
        try {
            ...
            for (int i = 0; i < measurementList.size(); i++) {
                if (!node.hasChild(measurementList.get(i))) {
                    throw new ProcessorException(String.format("Current deviceId[%s] does not contains measurement:%s",
                            deviceId, measurementList.get(i)));
                }
                MNode measurementNode = node.getChild(measurementList.get(i));
                if (!measurementNode.isLeaf()) {
                    throw new ProcessorException(
                            String.format("Current Path is not leaf node. %s.%s", deviceId, measurementList.get(i)));
                }
            ...
            case GRANT_ROLE:
                for (int i : permissions) {
                    if (!authorizer.grantPrivilegeToRole(roleName, nodeName.getFullPath(), i)) {
                        throw new ProcessorException("Role " + roleName + " already has " + PrivilegeType.values()[i]
            ...
            case GRANT_USER:
                for (int i : permissions) {
                    if (!authorizer.grantPrivilegeToUser(userName, nodeName.getFullPath(), i)) {
                        throw new ProcessorException("User " + userName + " already has " + PrivilegeType.values()[i]
                                + " on " + nodeName.getFullPath());
                    }
                }
                return true;
            ...
            case REVOKE_USER:
                for (int i : permissions) {
                    if (!authorizer.revokePrivilegeFromUser(userName, nodeName.getFullPath(), i)) {
                        throw new ProcessorException(
                                "User " + userName + " does not have " + PrivilegeType.values()[i] + " on " + nodeName);
                    }
                }
                return true;
            case REVOKE_ROLE:
                for (int i : permissions) {
                    if (!authorizer.revokePrivilegeFromRole(roleName, nodeName.getFullPath(), i)) {
                        throw new ProcessorException(
                                "Role " + roleName + " does not have " + PrivilegeType.values()[i] + " on " + nodeName);
                    }
                }
                return true;
                    ...
                    for (Path p : deletePathList) {
                        ...
                        if (subPaths.isEmpty()) {
                            throw new ProcessorException(String
                                    .format("There are no timeseries in the prefix of %s seriesPath", p.getFullPath()));
                        }
