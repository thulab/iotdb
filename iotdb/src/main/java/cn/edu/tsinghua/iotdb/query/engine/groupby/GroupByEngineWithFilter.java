...
public class GroupByEngineWithFilter {
    ...
    public GroupByEngineWithFilter(List<Pair<Path, AggregateFunction>> aggregations, List<FilterStructure> filterStructures,
        ...
        this.longInterval = IntervalExtractor.extractLongInterval(intervals);
...
//                        new BatchData(aggregations.get(i).right.dataType, true, true));
...
//        return groupByResult;
        ...
        for (Pair<Path, AggregateFunction> pair : aggregations) {
            ...
            if (aggregateTimestamps.size() == 0) {
                if (data != null) {
                    ...
                    data = new BatchData(aggregateFunction.dataType, true);
     ...
     */
    private static BatchData getDataUseSingleValueFilter(SeriesFilter queryValueFilter,
                                                                    BatchData res, int fetchSize, int valueFilterNumber)
            throws ProcessorException, IOException, PathErrorException {

        String deltaObjectUID = queryValueFilter.getSeriesPath().getDevice();
        String measurementUID = queryValueFilter.getSeriesPath().getMeasurement();
