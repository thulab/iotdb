...
public class PriorityMergeReaderByTimestamp extends PriorityMergeReader<PrioritySeriesReaderByTimestamp> implements SeriesReaderByTimeStamp {

  private long currentTimestamp;
  private boolean hasCachedTimeValuePair;
  private TimeValuePair cachedTimeValuePair;

  public PriorityMergeReaderByTimestamp(PrioritySeriesReaderByTimestamp... readers) throws IOException {
    super(readers);
    currentTimestamp = Long.MIN_VALUE;
    hasCachedTimeValuePair = false;
  }

  public PriorityMergeReaderByTimestamp(List<PrioritySeriesReaderByTimestamp> readers) throws IOException {
    super(readers);
  ...
  private void updateHeap(Element top) throws IOException {
    while (heap.size() > 0 && heap.peek().timeValuePair.getTimestamp() == top.timeValuePair.getTimestamp()) {
      Element e = heap.poll();
      PrioritySeriesReaderByTimestamp priorityTimeValuePairReader = readerList.get(e.index);
