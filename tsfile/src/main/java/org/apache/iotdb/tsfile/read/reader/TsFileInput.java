...
public interface TsFileInput {
   ...
   *
   * <p>
   * Setting the position to a value that is greater than the input's current size is legal but
   ...
   *
   * @param newPosition
   *            The new position, a non-negative integer counting the number of bytes from the
   *            beginning of the TsFileInput
   *
   ...
   *
   * <p>
   * This method works in the same manner as the {@link #read(ByteBuffer)} method, except that
   ...
   * greater than the TsFileInput's current size then no bytes are
   * read.
   * </p>
   *
   * @param dst
   ...
   *             If another thread interrupts the current thread while the read operation is in
   *             progress, thereby closing the channel and setting the current thread's interrupt
   *             status
   *
   * @throws IOException
   *             If some other I/O error occurs
