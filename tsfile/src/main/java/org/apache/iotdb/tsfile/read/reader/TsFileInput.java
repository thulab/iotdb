...
public interface TsFileInput {
   ...
   *
   * <p>
   * Setting the position to a value that is greater than the input's current size is legal but does
   * not change the size of the TsFileInput. A later attempt to read bytes at such a position will
   * immediately return an end-of-file indication.
   * </p>
   *
   ...
   * are read starting at the given position rather than at the TsFileInput's current position. This
   * method does not modify this TsFileInput's position. If the given position is greater than the
   * TsFileInput's current size then no bytes are read.
   * </p>
   *
