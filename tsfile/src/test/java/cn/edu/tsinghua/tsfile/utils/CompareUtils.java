...
public class CompareUtils {
    ...
    public static void isTimeSeriesChunkMetadataEqual(ChunkMetaData metadata1,
        ...
        if (CompareUtils.isTwoObjectsNotNULL(metadata1, metadata2, "ChunkMetaData")) {
            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getMeasurementUID(), metadata2.getMeasurementUID(),
                    "sensorUID")) {
            ...
            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getDigest(), metadata2.getDigest(), "digest")) {
                CompareUtils.isMapBufferEqual(metadata1.getDigest().getStatistics(), metadata2.getDigest().getStatistics(), "statistics");
            }
        }
    }

    public static void isTsDeviceMetadataEqual(TsDeviceMetadata metadata1, TsDeviceMetadata metadata2) {
        if (CompareUtils.isTwoObjectsNotNULL(metadata1, metadata2, "DeviceMetaData")) {
            assertEquals(metadata1.getStartTime(), metadata2.getStartTime());
            assertEquals(metadata1.getEndTime(), metadata2.getEndTime());

            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getChunkGroups(), metadata2.getChunkGroups(),
                ...
                for (int i = 0; i < metadata1.getChunkGroups().size(); i++) {
                    CompareUtils.isChunkGroupMetaDataEqual(metadata1.getChunkGroups().get(i),
    ...
    public static void isChunkGroupMetaDataEqual(ChunkGroupMetaData metadata1, ChunkGroupMetaData metadata2) {
        if (CompareUtils.isTwoObjectsNotNULL(metadata1, metadata2, "ChunkGroupMetaData")) {
            assertTrue(metadata1.getDeviceID().equals(metadata2.getDeviceID()));

            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getChunkMetaDataList(), metadata2.getChunkMetaDataList(),
                ...
                for (int i = 0; i < metadata1.getChunkMetaDataList().size(); i++) {
                    CompareUtils.isTimeSeriesChunkMetadataEqual(metadata1.getChunkMetaDataList().get(i),
    ...
    public static void isTsDeviceMetadataIndexEqual(TsDeviceMetadataIndex index1, TsDeviceMetadataIndex index2) {
        if (CompareUtils.isTwoObjectsNotNULL(index1, index2, "TsDeviceMetadataIndex")) {
    ...
    public static void isFileMetaDataEqual(TsFileMetaData metadata1, TsFileMetaData metadata2) {
        if (CompareUtils.isTwoObjectsNotNULL(metadata1, metadata2, "File MetaData")) {
            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getDeviceMap(), metadata2.getDeviceMap(),
                ...
                for (String key : deviceMetadataMap1.keySet()) {
                    CompareUtils.isTsDeviceMetadataIndexEqual(deviceMetadataMap1.get(key), deviceMetadataMap2.get(key));
                }
            }

            if (CompareUtils.isTwoObjectsNotNULL(metadata1.getMeasurementSchema(), metadata2.getMeasurementSchema(),
