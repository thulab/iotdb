...
class ConverterTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("testToSparkSqlSchema") {
    val fields: util.ArrayList[ColumnSchema] = new util.ArrayList[ColumnSchema]()
    fields.add(new ColumnSchema("s1", TSDataType.INT32, TSEncoding.PLAIN))
    fields.add(new ColumnSchema("s2", TSDataType.INT64, TSEncoding.PLAIN))
    fields.add(new ColumnSchema("s3", TSDataType.FLOAT, TSEncoding.PLAIN))
    fields.add(new ColumnSchema("s4", TSDataType.DOUBLE, TSEncoding.PLAIN))
    fields.add(new ColumnSchema("s5", TSDataType.BOOLEAN, TSEncoding.PLAIN))
    fields.add(new ColumnSchema("s6", TSDataType.TEXT, TSEncoding.PLAIN))
    val columns = new ArrayBuffer[String]()
    columns += SQLConstant.RESERVED_DELTA_OBJECT
